(function () {
  ...
  const ENGLISH = Object.freeze({
    ...
    '默认模型': 'Default model',
    '↑ 并入 Excel': '↑ Merge Excel',
    '↓ 导出 Excel': '↓ Export Excel',
    '＋ 新建表': '+ New table',
    '添加数据文件': 'Add data file',
    '移除附件': 'Remove attachment',
    '上传数据库、CSV 或 Excel，并结合本条指令处理': 'Upload a database, CSV, or Excel file and process it with this instruction.',
    '发送后先判断任务，再安全接入数据': 'The task is routed first, then the data is attached safely.',
    '发送后由模型判断任务入口；写入仍需确认': 'The model routes the task first; writes still require confirmation.',
    '附件已就绪，请在输入框说明要完成的数据库任务': 'Attachment ready. Describe the database task in the composer.',
    '请在输入框说明要对这个文件完成什么数据库任务': 'Describe the database task you want to run on this file.',
    '仅支持 SQLite、CSV 和 XLSX 数据文件': 'Only SQLite, CSV, and XLSX data files are supported.',
    '不能上传空文件': 'Empty files cannot be uploaded.',
    '每条任务只能添加一个数据文件': 'Each task can include one data file.',
    '文件已接入，正在执行你的数据库任务': 'The file is attached. Running your database task now.',
    '正在上传：': 'Uploading: ',
    '正在检查数据库：': 'Inspecting database: ',
    '正在判断文件任务：': 'Routing file task: ',
    '文件任务失败：': 'File task failed: ',
    '输入问题，或先打个招呼…': 'Ask a question, or start with a greeting…',
    '发送': 'Send',
    'Enter 发送 · Shift+Enter 换行 · Excel 可作为新数据库接入，也可预检后并入当前库': 'Enter to send · Shift+Enter for a new line · Attach Excel as a new database or validate and merge it into the current one.',
    '可拖入 SQLite / CSV / XLSX，并在同一条消息里说明查询、分析、清洗、写入或并入要求': 'Drop a SQLite, CSV, or XLSX file here and describe the query, analysis, cleanup, edit, or merge in the same message.',
    '并入当前数据库': 'Merge into current database',
    '工作表对应已有表，首行为物理字段名；预检不会写入数据库': 'Each worksheet maps to an existing table and row one uses physical column names. Validation does not write to the database.',
    '单一事务': 'Single transaction',
    '确认时重新校验文件、权限与数据库结构；任一工作表失败会全部回滚。': 'Confirmation revalidates the file, permissions, and schema. Any worksheet failure rolls back the entire import.',
    '确认并入': 'Confirm merge',
    '简要过程': 'Brief process',
    '阶段与真实观测摘要，不含模型隐藏思维链': 'Stages and verified observations only; hidden model reasoning is never exposed.',
    '记忆使用': 'Memory used',
    '记忆召回': 'Memory recall',
    '分层记忆': 'Layered memory',
    '清空当前范围': 'Clear current scope',
    '工作检查点': 'Working checkpoint',
    'L0 · 记忆宪章': 'L0 · Memory policy',
    'L1 · 索引': 'L1 · Index',
    'L2 · 已验证路径事实': 'L2 · Verified route facts',
    'L3 · 只读策略': 'L3 · Read-only strategies',
    'L4 · 执行片段': 'L4 · Execution episodes',
    '记忆宪章': 'Memory policy',
    '紧凑索引': 'Compact index',
    '已验证路径事实': 'Verified route facts',
    '只读策略': 'Read-only strategies',
    '执行片段': 'Execution episodes',
    '反思记录': 'Reflection records',
    '模型判断中': 'Model deciding',
    '已准入': 'Admitted',
    '部分准入': 'Partially admitted',
    '仅 L4': 'L4 only',
    '已舍弃': 'Discarded',
    '反思失败': 'Reflection failed',
    '旧版记录': 'Legacy record',
    '待判断': 'Pending',
    '需重试': 'Retry needed',
    '模型反思': 'Model reflection',
    '模型建议': 'Model proposal',
    '实际写入': 'Effective write',
    '重试模型反思': 'Retry model reflection',
    '已重新加入模型反思队列': 'Queued for model reflection again',
    '已舍弃的脱敏事件': 'Discarded redacted event',
    '正在异步评估长期价值和可复用层级。': 'Evaluating long-term value and reusable layers in the background.',
    '本次模型反思未完成，数据库回答不受影响。': 'This model reflection did not finish; the database answer is unaffected.',
    '无补充理由。': 'No additional reason.',
    '新颖度': 'Novelty',
    '可复用': 'Reusability',
    '置信度': 'Confidence',
    '本轮记忆': 'This turn',
    '已进入模型反思队列': 'Queued for model reflection',
    '反思待重试': 'Reflection needs retry',
    '尚未代表已进入长期层；最终结果由模型提议与本地门禁共同决定': 'This is not yet long-term memory; the model proposal and local gate determine the final result.',
    '本轮召回来源': 'Memory recalled this turn',
    '候选': 'Candidate',
    '已晋级': 'Promoted',
    '已停用': 'Disabled',
    '停用': 'Disable',
    ...
    '已被纠错': 'Corrected',
    '已纠错': 'Corrected',
    '代码固定 · 不可写': 'Code-defined · immutable',
    '只加载与当前问题相关的分层引用，不把完整历史塞进模型上下文。': 'Only references relevant to the current question are loaded; full histories never enter the model context.',
    '暂无执行验证的路径事实': 'No execution-verified route facts yet',
    '首次或每次纠错后取得 3 次成功证据才会晋级': 'Promotion requires three successful executions initially and again after every correction',
    '暂无脱敏执行记录': 'No redacted execution episodes yet',
    '边界规则不可用': 'Memory policy is unavailable',
    '正在加载边界规则': 'Loading memory policy',
    '由当前问题重新绑定': 'Rebound from the current question',
    '无可复用动作': 'No reusable action',
    '模型异步判断是否值得记忆并提议层级；本地证据门禁仍决定最终写入。L0 不可写，L1 由程序生成，失败或已纠错的事件不能进入 L2/L3。': 'The model asynchronously decides whether an event is worth remembering and proposes layers; the local evidence gate still controls final admission. L0 is immutable, L1 is program-generated, and failed or corrected events cannot enter L2/L3.',
    '只有完成的数据库执行证据可以强化长期记忆': 'Only completed database execution evidence can strengthen long-term memory',
    '记忆严格绑定数据库、授权范围和结构版本': 'Memory is strictly bound to the database, access scope, and schema version',
    '不保存 SQL、结果行、连接信息、凭据或模型提示词': 'SQL, result rows, connection details, credentials, and model prompts are never stored',
    '策略只能描述白名单只读动作，不能包含可执行代码': 'Strategies may only describe allowlisted read-only actions and cannot contain executable code',
    '模型只能提出是否记忆及目标层，最终写入由本地证据门禁决定': 'The model may only propose whether to remember and target layers; the local evidence gate controls final writes',
    '纠错会降权；首次或每次纠错后均需三次成功证据才能晋级': 'Corrections reduce confidence; promotion requires three successful executions initially and after every correction',
    '会话工作记忆': 'Session working memory',
    '话题锚点': 'Topic anchor',
    '数据库语义记忆': 'Database semantic memory',
    '跨会话个人记忆': 'Cross-session personal memory',
    '当前会话': 'Current session',
    '当前数据库（持久化）': 'Current database (persistent)',
    '未启用': 'Not enabled',
    '意图': 'Intent',
    '动作': 'Action',
    '观测': 'Observation',
    ...
    '数据库写操作': 'Database write',
    '基础沟通': 'Conversation',
    '接收请求并判断意图': 'Receive request and classify intent',
    '加载数据库上下文': 'Load database context',
    '只读执行': 'Read-only execution',
    '受控写入': 'Controlled write',
    '启动有界数据库探索': 'Start bounded database exploration',
    '检查相关表结构': 'Inspect relevant schemas',
    '搜索表和字段': 'Search tables and columns',
    '跨表搜索真实值': 'Search real values across tables',
    '抽样核对数据': 'Sample and verify data',
    '核对表间关系': 'Verify table relationships',
    '执行受控只读查询': 'Run a controlled read-only query',
    '召回数据库证据': 'Retrieve database evidence',
    '验证查询假设': 'Validate query hypothesis',
    '准备归纳回答': 'Prepare answer synthesis',
    '归纳数据库证据': 'Synthesize database evidence',
    '整理回答与报告': 'Prepare answer and report',
    '回答与报告已生成': 'Answer and report generated',
    '已把短追问绑定到最近的完整问题': 'Bound the short follow-up to the latest complete question',
    '本轮未继承上一问题': 'No topic inherited from the previous question',
    '本轮没有可用的历史消息': 'No prior messages were available for this answer',
    '不会自动提取或跨会话保存个人偏好与事实': 'Personal preferences and facts are not automatically extracted or saved across conversations',
    '将格式合规的 Excel 预检后并入当前数据库': 'Validate a conforming Excel workbook before merging it into the current database.',
    '把当前权限范围内的数据导出为多工作表 Excel': 'Export the currently authorized data to a multi-sheet Excel workbook.',
    '当前远程数据库以只读模式连接': 'The current remote database is connected in read-only mode.',
    '当前会话不能执行 Excel 批量写入': 'This session cannot perform batch Excel writes.',
    '完整脱敏事件': 'Complete redacted event',
    '账本不保存原始问题、SQL、结果行或连接凭据': 'The ledger never stores original questions, SQL, result rows, or connection credentials.',
    '数据库': 'Database',
    '手动刷新': 'Refresh',
  ...
  const DYNAMIC_ENGLISH = Object.freeze([
    ...
    [/^(\d+)\s*条有效记忆 · (\d+)\s*条反思记录$/u, (_, active, total) => `${active} active memories · ${total} reflection records`],
    [/^(\d+)\s*个引用$/u, (_, count) => `${count} references`],
    [/^(\d+)\s*张表$/u, (_, count) => `${count} tables`],
    [/^(\d+)\s*张目标表$/u, (_, count) => `${count} target tables`],
    [/^(\d+)\s*行待新增$/u, (_, count) => `${count} rows to insert`],
    [/^(\d+)\s*\/\s*(\d+)\s*层在本轮生效$/u, (_, active, total) => `${active} of ${total} layers used`],
    [/^(\d+)\s*\/\s*(\d+)\s*层有本轮输入$/u, (_, active, total) => `${active} of ${total} layers had input this turn`],
    [/^本轮参考最近\s*(\d+)\s*条消息$/u, (_, used) => `Used the latest ${used} messages`],
    [/^召回\s*(\d+)\s*个分层引用$/u, (_, count) => `Recalled ${count} layered references`],
    [/^命中\s*(\d+)\s*条路径事实$/u, (_, count) => `Matched ${count} verified route facts`],
    [/^命中\s*(\d+)\s*条策略，其中\s*(\d+)\s*条已晋级$/u, (_, count, promoted) => `Matched ${count} strategies; ${promoted} promoted`],
    [/^召回\s*(\d+)\s*条相关执行记录$/u, (_, count) => `Recalled ${count} related execution episodes`],
    [/^当前数据库 · 当前授权 · 结构\s+([0-9a-f]+)$/u, (_, fingerprint) => `Current database · current access · schema ${fingerprint}`],
    [/^当前数据库 · 当前授权 · 结构\s+([0-9a-f]+) · (\d+) 条旧结构记录未参与召回$/u, (_, fingerprint, count) => `Current database · current access · schema ${fingerprint} · ${count} old-schema episodes excluded from recall`],
    [/^(\d+) 次支持 · (\d+) 次纠错$/u, (_, support, corrections) => `${support} supports · ${corrections} corrections`],
    [/^(\d+) \/ (\d+) 次支持 · (\d+) 次纠错$/u, (_, support, threshold, corrections) => `${support} / ${threshold} supports · ${corrections} corrections`],
    [/^触发：(.+)$/u, (_, detail) => `Triggers: ${detail}`],
    [/^目标：(.+)$/u, (_, detail) => `Targets: ${detail}`],
    [/^结果 (\d+) · 证据 (\d+)$/u, (_, results, evidence) => `Results ${results} · evidence ${evidence}`],
    [/^结果 (\d+) · 证据 (\d+) · 已被纠错$/u, (_, results, evidence) => `Results ${results} · evidence ${evidence} · corrected`],
    [/^模型建议\s+(.+)$/u, (_, layers) => `Model proposal ${layers}`],
    [/^实际写入\s+(.+)$/u, (_, layers) => `Effective write ${layers}`],
    [/^本轮参考最近\s*(\d+)\s*条消息（上限\s*(\d+)\s*条）$/u, (_, used, limit) => `Used the latest ${used} messages (limit ${limit})`],
    [/^命中\s*(\d+)\s*项业务定义；目录共\s*(\d+)\s*项$/u, (_, hits, total) => `Matched ${hits} business definitions from a catalog of ${total}`],
    [/^本轮未命中业务定义；目录共\s*(\d+)\s*项$/u, (_, total) => `No business definition matched; the catalog contains ${total}`],
    [/^工作表\s+(.+)$/u, (_, sheet) => `Worksheet ${sheet}`],
    [/^已完成观测：(\d+) 项结果$/u, (_, count) => `Observation complete: ${count} results`],
    [/^(.+)返回\s*(\d+)\s*项$/u, (_, action, count) => `${ENGLISH[action] || action} returned ${count} results`],
    [/^意图判断完成：(.+)$/u, (_, intent) => `Intent classified: ${ENGLISH[intent] || intent}`],
    [/^正在上传并预检 Excel：(.+)$/u, (_, name) => `Uploading and validating Excel: ${name}`],
