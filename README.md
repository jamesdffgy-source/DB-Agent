...
[![Project gate](https://github.com/jamesdffgy-source/DB-Agent/actions/workflows/ci.yml/badge.svg)](https://github.com/jamesdffgy-source/DB-Agent/actions/workflows/ci.yml)
[![Latest release](https://img.shields.io/github/v/release/jamesdffgy-source/DB-Agent?display_name=tag&style=flat-square)](https://github.com/jamesdffgy-source/DB-Agent/releases)
...
![Hand-drawn overview of the DB-Agent workflow](docs/assets/dbagent-handdrawn-workflow.png)

DB-Agent turns a request into a typed, reviewable database operation before anything reaches the database. Read paths are physically read-only and bounded. Write paths stop at validation and a change preview until the user explicitly confirms them.
...
![DB-Agent desktop interface](docs/assets/dbagent-overview.png)

1. Attach a SQLite database, import CSV/`.xlsx`, or add a read-only MySQL/PostgreSQL connection.
...
- CPython 3.12 (CI and the lock file pin 3.12.13);
...
Attach `demo_data/dbagent_demo.sqlite`, then follow the [demo guide](docs/DEMO_TEST_GUIDE.md). For manual setup, offline constraints, WebView2 help, and clean-machine verification, read the [installation guide](docs/INSTALLATION.md).
...
| MySQL 8.4 | Schema, PK/FK discovery, bounded queries, grouped metrics, timeout and read-only enforcement | Read-only; no controlled writes |
| PostgreSQL 17 | Schema, PK/FK discovery, bounded queries, grouped metrics, timeout and read-only enforcement | Read-only; no controlled writes |
...
The desktop client talks only to a loopback `aiohttp` service protected by a local token and same-origin checks. SQLite reads use physical read-only connections plus `query_only`; write confirmation is bound to one database, one reviewed plan, and one use.
...
DB-Agent is released under the [MIT License](LICENSE). Redistributed assets and their licenses are listed in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). The hand-drawn project artwork is covered by the repository license; its provenance is recorded in [docs/assets/README.md](docs/assets/README.md).
