...
- 直接运行依赖：aiohttp 3.14.3、pywebview 6.2.1、openpyxl 3.1.5、requests 2.34.2、tzdata 2026.3、
...
- `requirements.txt` 固定 7 个直接依赖；`requirements.lock` 固定 Windows x64 / CPython 3.12
  ...
  运行 `pip check`；`.python-version` 选择受支持的 3.12 系列。
...
- 发布目标：Windows 10/11 x64，CPython 3.12；`.python-version` 和 CI 选择可用的 3.12 补丁版本，锁文件包含
