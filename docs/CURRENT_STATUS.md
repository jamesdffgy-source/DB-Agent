...
- 使用 DeepSeek V4 Flash 和同一个 51,863,552 字节导师数据库实测：“肖仰华老师的工作是什么”→“是什么”→“证据呢”
  三轮连续追问都只命中 `demo_research` 的同一条肖仰华记录；复杂问法能区分已知归属单位与未知岗位/研究方向/证据，
  并如实解释 `unresolved` 和空值。本轮实体/记忆修复后 430/430 完整回归通过。
