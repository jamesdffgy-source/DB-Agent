  ...
  └─ Vanilla HTML/CSS/JavaScript + ECharts
       └─ loopback aiohttp desktop_bridge
            ...
            └─ DBQuillAgent
                 ...
                 ├─ 类型化关系计划与 NL-to-SQL
                 ├─ 检索、只读 OperationGraph 与有界观察—重规划
                 └─ 写校验 → 回滚预览 → 一次性确认 → 事务执行
...
- 通用读路将初始意图视为可修正假设：对空检索、实体窄投影、计划表/证据表错位、部分组合失败或明确继续核验请求，
  模型可在真实 schema 和前一步观测上选择查结构、召回证据、自然语言查询、完成或停止。默认预算为 3 次规划、2 个工具动作和 25 秒；写路不进入。
...
- 前一个 `advisor_research_demo.db` 由用户确认为误上传，只保留为空值/空结果回归样本，不再用它判断目标数据。正确的 `fudan_cs_research.db` 含 33 张表；
  ...
  DeepSeek V4 Flash 实测“肖仰华老师的工作”首先返回两个含义选项；回复“我的意思是他担任的工作”后，首条证据为 `scholars` 规范人物行并正确返回上述三项职务。有界重规划与实体对齐完成后，简单“肖仰华担任哪些职务”真实复测用时 6.91 秒，回答覆盖教授、博导、实验室主任和期刊编委职务。当前完整回归为 445/445。
...
6. 有界观察—重规划已覆盖高信号失败/不完整形状，但不是无条件的开放域自治搜索；未命中触发条件的表达仍可能由首个执行器直接回答。实体仅通过数字主外键延展到论文/项目时，仍需实体关系扩展计划，不允许检索器猜 JOIN。
