...
- 对话输入区可把单个 SQLite/CSV/`.xlsx` 与同一条自然语言任务一起发送，也支持拖放。附件在传输前先进入
  类型化文件任务路由；CSV/SQLite 和普通 XLSX 作为独立数据源接入后继续执行查询、分析、清洗或受控写入，
  明确要求并入当前库的 XLSX 则只进入既有预检与确认链，路由本身不写数据库。
...
- 问答运行公开意图、动作、观测和归纳的阶段时间线，但不暴露模型隐藏思维链。工作检查点与长期层分离；
  L0 固定记忆宪章，L1 为程序生成的紧凑引用索引，L2 为执行验证的路径事实，L3 为候选/晋级/可停用的类型化只读策略，
  L4 为脱敏执行片段。完成的读取会先异步请模型判断是否值得记忆及建议层，本地证据门禁再决定实际层；失败/已纠错事件不能进入 L2/L3，低置信建议会降级。后台队列未启用或不可用时事件直接进入 `failed` 而不永久停在 `pending`。首次及每次纠错后仍需要三次有效成功证据才晋级；审计和记忆工作台可区分模型建议、实际写入、门禁原因与失败重试。
...
- 异步记忆反思定向验证已覆盖严格模型合同、待判断不提前召回/强化、本地降级/舍弃、反思期间纠错、v1 `legacy` 迁移、建议/准入审计、失败可见与 operator 重试。真实 DeepSeek V4 Flash 将一次普通计数判为 `discard` 并无长期写入，将一次跨表关系检查路线判为 L2，本地有效层为 L4+L2；两次临时验证均确认结果行未持久化。
...
- 当前正式门禁为 105 个受控文件、493/493 回归、134/134 离线评测、8/8 时区探针和 12/12 历史脱敏模型基线；文件任务路由、任务上下文、三格式上传、RBAC、前端组合与异步记忆边界均包含在回归中。JavaScript、仓库卫生、登记时间和源码指纹以 `PROJECT_STATE.json` 为准。
